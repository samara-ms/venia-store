...
document.addEventListener("DOMContentLoaded", function() {
    ...
    const handleScroll = ()=>{
        if (window.scrollY > 100) header.classList.add("nav-header--fixed");
        else header.classList.remove("nav-header--fixed");
    };
    ...
    if (mobileFilterBtn && productFilterElt) mobileFilterBtn.addEventListener("click", function() {
...
const Plp = {
    ...
    async getProductsList (sortBy = "", category = [], searchQuery = "") {
        try {
            ...
            if (category.length > 0) products = products.filter((product)=>category.includes(product.category));
            if (searchQuery) products = products.filter((product)=>product.title.toLowerCase().includes(searchQuery));
            displayedProducts = products.slice(0, itemsPerPage * currentPage);
            Plp.displayProducts(displayedProducts);
            Plp.setupPagination();
            document.getElementById("loading").style.display = "none";
            if (products.length > displayedProducts.length) document.getElementById("loadMore").style.display = "block";
                ...
                productHTML += `<li class="product__product-card">
                    ...
                    <div class="product__product-content">
                        ...
                        <button class="product__add-to-cart">
                            <img src="${0, _heartPngDefault.default}" alt="cart icon" class="product__cart-icon">
                        </button>
                    </div>
                </li>`;
            });
    ...
    setupPagination () {
        try {
            ...
            if (displayedProducts.length >= products.length) paginationList.style.display = "none";
            else for(let i = 1; i <= totalPages; i++){
...
document.getElementById("loadMore").addEventListener("click", function() {
    ...
    } else document.querySelector(".product__pagination").style.display = "block";
});
// Calling
Plp.getProductsList(sortBy, category, searchQuery);
Plp.categoryFilter();
...
},{"./assets/images/heart.png":"lhJip","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"lhJip":[function(require,module,exports) {
module.exports = require("f5675bf6e2eec85").getBundleURL("bLxZJ") + "heart.99631e2c.png" + "?" + Date.now();

},{"f5675bf6e2eec85":"lgJ39"}],"lgJ39":[function(require,module,exports) {
"use strict";
var bundleURL = {};
